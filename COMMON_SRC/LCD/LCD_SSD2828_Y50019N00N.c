...
bool bLCD_MIPI_SSD2828_Init (void)
{
	...
	LCD_SSD2828_RegWr(0xB3, 0x0F18);   // VFP = 16,HFP = 44         
	...
	LCD_SSD2828_RegWr(0xBA,0x8024);   // 10-251<Fout<500,MS=1,NS=22                                                                    
	LCD_SSD2828_RegWr(0xBB,0x0005);   // LP mode clock, Divide by 6                                                                    
	LCD_SSD2828_RegWr(0xD5,0x1860);                                                                                                      
	LCD_SSD2828_RegWr(0xC9,0x1203);   // HS Zero Delay = HZD * nibble_clk ; HS Prepare Delay =  4 nibble_clk + HPD * nibble_clk        
	LCD_SSD2828_RegWr(0xCA,0x1D02);   // CLK Zero Delay = CZD * nibble_clk ; CLK Prepare Delay =  3 nibble_clk + CPD * nibble_clk      
	LCD_SSD2828_RegWr(0xCB,0x041F);   // CLK Pre Delay = CPED * nibble_clk + 0-1 * lp_clk(min,max) ; CLK Post Delay = CPTD * nibble_clk
	LCD_SSD2828_RegWr(0xCC,0x080A);   // CLK Trail Delay = CTD * nibble_clk ; HS Trail Delay = HTD * nibble_clk                        
...
void LCD_MIPI_SSD2828 (void)
{
	...
	//! Timing
	LCD->LCD_HT_DM_SIZE = 24;
	LCD->LCD_VT_DM_SIZE = 12;
	LCD->LCD_HT_START = 24 + 24;
	LCD->LCD_VT_START = 15 + 4;
