...
static uint8_t ubKNL_WakeUpFlag[4] = {0,0,0,0};
...
#define KNL_MINORVER    3        //!< Minor version = 3
...
static void KNL_AdoEncMonitThread(void const *argument)
{
	...
	while(1)
	{
		...
		else if(tAdoInfo.Type == BOTH)
		{
			printd(DBG_Debug3Lvl, "----- Both -----\r\n");
			printd(DBG_Debug3Lvl, "AdoEncQ(Audio32) : Addr:0x%x,SZ:0x%x\r\n",tAdoInfo.Audio32Addr,tAdoInfo.Audio32Size);
			printd(DBG_Debug3Lvl, "AdoEncQ(AAC) : Addr:0x%x,SZ:0x%x\r\n",tAdoInfo.AACAddr,tAdoInfo.AACSize);
			printd(DBG_ErrorLvl, "Without Support !!!!\r\n");
			continue;//while(1);
		}
		...
		if(KNL_SRC_NONE != tKNL_AdoSrcNum)
		{
	...
	while(1)
	{
		...
		switch(tKNL_DecEvent)
		{
			...
			case PLAY_BUF_EMP:
				...
				{
					ADO_Audio32_Decoder_UnInit(i);
					ADO_Audio32_Decoder_Init(i,ADO_GetAudio32DecFormat(i));
				}
				printd(DBG_ErrorLvl, "KNL_AdoDecMonitThread-->Aud32 dec unit/init\r\n");
				break;
		}
	}
}
...
void KNL_BlockInit(void)
{
	...
	if(ubNodeExist && (ubKNL_InitBBFlg == 0))
	{
		...
		uint8_t ubKNL_PsValue = 0, ubKNL_WorSts = 1;
...
#define RECORD_PS_ADDR	0
#define PS_ECO_VALUE	0x30
		ubKNL_PsValue  = wRTC_ReadUserRam(RECORD_PS_ADDR);
		ubKNL_PsValue &= 0xF0;
		KNL_DisableWORFunc();
		if((!ubRTC_GetKey()) && (ubKNL_PsValue == PS_ECO_VALUE))
		{			
			BB_Init(tKNL_BbSlotMode, NULL, 0);	//Step(n)
			ubKNL_WorSts = BB_ConfirmWakeUpInf();
		}
		else
			BB_Init(tKNL_BbSlotMode, NULL, 1);	//Step(n)
...
//		osThreadCreate(osThread(ImgMonitThread), NULL);
	...
	if((ubKNL_ChkExistNode(KNL_NODE_ADC) || ubKNL_ChkExistNode(KNL_NODE_DAC)) && (ubKNL_InitAdoFlg == 0))
	{
		...
		ADO_SetSigmaDeltaAdcGain(ADO_SIG_BOOST_0DB, ADO_SIG_PGA_0DB);
		//---------------------------------
		// ADC djust functions
		//---------------------------------
...
//		sLcdInfor.uwLcdOutputVsize = uwLCD_GetLcdVoSize();		
		...
		else if(tKNL_GetDispRotate() == KNL_DISP_ROTATE_90)
		{
			...
			sLcdInfor.tChRes[1].uwCropVstart = 0;
			sLcdInfor.tChRes[1].uwCropHsize  = uwKNL_GetVdoV(ubDisp2Src);
			sLcdInfor.tChRes[1].uwCropVsize  = uwKNL_GetVdoH(ubDisp1Src);
		}
		pLcdCropScaleParam = (LCD_INFOR_TYP *)(&sLcdInfor);
	}	
		...
		else if(ubBbStartFlg && (!ubBbLinkFlg))
		{
			...
			if(ubKNL_ExistNode(tProc.ubSrcNum,KNL_NODE_H264_ENC))
			{
				...
				if(KNL_GetTuningToolMode() == KNL_TUNINGMODE_OFF) 
            	    ubKNL_ResetIFlg = 1;	//! H264_ResetIPCnt((H264_ENCODE_INDEX)tNodeInfo.ubCodecIdx);
			}
		}
		//Case 3
...
RTYSEND_ADO:
	...
	if(ubNextNode == KNL_NODE_COMM_TX_ADO)
	{
		...
		if((ubTotalBufNum-ubUsedBufNum) >= 1)
			ubBufOkFlg = 1;
		if((ubBB_GetStartFlg() == BB_TASK_START) && (ubKNL_GetCommLinkStatus(ubLinkRole) == BB_LINK) && (ubBufOkFlg))
		{
			...
			if(!ubAdoRtyCnt)
			{
				...
				ulTempAddr = ulBUF_GetAdcFreeBuf();						//Get BitStream Buffer
...
static void KNL_CommLinkUpdateThread(void const *argument)
{
	...
	uint8_t ubLinkLoopCycle = 1;//10;	//1;
	uint8_t ubLinkTh    	= 1;//4;	//1;
	uint8_t ubLinkCnt   	= 0;
	uint8_t ubSrcNumMap1,ubSrcNumMap2,ubSrcNumMap3;
	uint8_t ubKNL_StartUpLhFlag = FALSE;

	tProc = tProc;
	ubSrcNumMap1 = ubSrcNumMap1;
	ubSrcNumMap2 = ubSrcNumMap2;
	ubSrcNumMap3 = ubSrcNumMap3;
	ubKNL_StartUpLhFlag	= ubKNL_StartUpLhFlag;
	for(i=0;i<6;i++)
	{
		...
		if((++i%ubLinkLoopCycle) == 0)
		{
			for(k=0;k<6;k++)
			{
				...
				if(ubLinkCnt >= ubLinkTh)
				{
					...
					#ifdef OP_STA
					if((ubKNL_SenStartFlg) && (ubSEN_GetFirstOutFlg()) && (TRUE == ubKNL_ImgStabFlg))
					{
						if(FALSE == ubKNL_StartUpLhFlag)
						{
							ubLinkTh			= 4;
							ubLinkLoopCycle		= 10;
							ubKNL_StartUpLhFlag = TRUE;
						}
						SEN_SetFirstOutFlg(0);
					}
					#endif
...
{
	TIMER_SETUP_t tTimer21Param;

	tTimer21Param.tCLK 		  = TIMER_CLK_EXTCLK;	
	tTimer21Param.ulTmLoad 	  = 10000 * ((1000 / ubKNL_GetVdoFps()) * 6);
	tTimer21Param.ulTmCounter = tTimer21Param.ulTmLoad;
	tTimer21Param.ulTmMatch1  = tTimer21Param.ulTmLoad + 1;
	tTimer21Param.tOF 		  = TIMER_OF_ENABLE;
	tTimer21Param.tDIR 		  = TIMER_DOWN_CNT;
	tTimer21Param.tEM 		  = TIMER_CB;
	tTimer21Param.pvEvent 	  = KNL_ImgStabEvent;
	TIMER_Start(TIMER2_1, tTimer21Param);
}
//------------------------------------------------------------------------------
...
void KNL_EnableWORFunc(void)
{
	...
	ubKNL_PsValue  = wRTC_ReadUserRam(RECORD_PS_ADDR);
	ubKNL_PsValue &= 0xF;
	ubKNL_PsValue |= PS_ECO_VALUE;
	RTC_WriteUserRam(RECORD_PS_ADDR, ubKNL_PsValue);
...
{
	LINK_ROLE tBB_LinkRole[4] = {[KNL_STA1] = BB_STA1,
								 [KNL_STA2] = BB_STA2,
								 [KNL_STA3] = BB_STA3,
								 [KNL_STA4] = BB_STA4};
