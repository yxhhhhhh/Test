...
void UI_KeyEventExec(void *pvKeyEvent)
{
    ...
    if (tUI_PuSetting.ubDefualtFlag == TRUE) //»Ö¸´³ö³§ÉèÖÃÖ»ÓÐÉÏÏÂ×óÓÒ,Enter,PowerKe   y¼üÓÐÓÃ
    {
...
void UI_SettingSubSubMenuEnterKey(uint8_t SubMenuItem)
{
    ...
    case DEFAULT_ITEM:
        if (ubDefualtFlag == 1)
        {
            ...
            tOsdImgInfo.uwXStart = 0;
            tOsdImgInfo.uwYStart = 0;
            OSD_EraserImg2(&tOsdImgInfo); //¸´Î»ÖØÆôÒÔºóOSD¿ÉÄÜ»áÓÐ²ÐÁô
...
/*
    tEnUI.tStatus = EN_FIX_HOPPING;                               //¹Ì¶¨ÆµµãÄ£Ê½

    tEnUI.tRole = SET_STA1;                                       //ÔÚSTA1¶ËÉúÐ§

    tEnUI.ubData1 = 2;                                            //ÔÚ¸ßÆµµãÉúÐ§£¬0¡¢1¡¢2´ú±íµÍÖÐ¸ßÆµµã

    tEN_Start(tEnUI.tStatus,tEnUI.tRole,&tEnUI.ubData1,NULL);     //¿ªÆô¹¤³ÌÄ£Ê½
*/
...
void UI_CheckUsbCharge(void)
{
    ...
    if(UI_GetUsbDet() == 0) //USB_DET
    {
        ...
        if (UI_GetBatChgFull() == 0) //µç³Ø³äÂú
        {
        }
    }
    else
