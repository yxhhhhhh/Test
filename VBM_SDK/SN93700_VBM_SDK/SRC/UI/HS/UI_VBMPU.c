...
void UI_SettingSubSubMenuEnterKey(uint8_t SubMenuItem)
{
    ...
    case DEFAULT_ITEM:
        if (ubDefualtFlag == 1)
        {
            ...
            tOsdImgInfo.uwXStart = 0;
            tOsdImgInfo.uwYStart = 0;
            OSD_EraserImg2(&tOsdImgInfo); //赂麓脦禄脰脴脝么脪脭潞贸OSD驴脡脛脺禄谩脫脨虏脨脕么
...
/*
    tEnUI.tStatus = EN_FIX_HOPPING;                               //鹿脤露篓脝碌碌茫脛拢脢陆

    tEnUI.tRole = SET_STA1;                                       //脭脷STA1露脣脡煤脨搂

    tEnUI.ubData1 = 2;                                            //脭脷赂脽脝碌碌茫脡煤脨搂拢卢0隆垄1隆垄2麓煤卤铆碌脥脰脨赂脽脝碌碌茫

    tEN_Start(tEnUI.tStatus,tEnUI.tRole,&tEnUI.ubData1,NULL);     //驴陋脝么鹿陇鲁脤脛拢脢陆
*/
...
void UI_CheckUsbCharge(void)
{
    ...
    {
        if (tUSBD_GetConfigStatus() == 1) //鲁盲碌莽脝梅
        {
            if (GPIO->GPIO_O12 == 1)
            {
                printd(Apk_DebugLvl, "UI_CheckUsbCharge AC Charge!\n");
                GPIO->GPIO_O12 = 0; //麓贸碌莽脕梅驴矛鲁盲
            }
        }
        else
        {
        ...
        if (UI_GetBatChgFull() == 0) //碌莽鲁脴鲁盲脗煤
        {
        }
    }
    else
